...
class ControllerExtensionPaymentWooppay extends Controller
{
	...
	public function index()
	{
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/payment/wooppay', 'token=' . $this->session->data['token'], 'SSL'),
			'separator' => ' :: '
		);

		$data['action'] = $this->url->link('extension/payment/wooppay', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['wooppay_service'])) {
			$data['wooppay_service'] = $this->request->post['wooppay_service'];
		} else {
			$data['wooppay_service'] = $this->config->get('wooppay_service');
		}
		...
		$this->response->setOutput($this->load->view('extension/payment/wooppay.tpl', $data));
	}
