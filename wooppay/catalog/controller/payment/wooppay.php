...
class ControllerPaymentWooppay extends Controller
{
    public function index()
    {
        ...
        $data['button_confirm_action'] = $this->url->link('payment/wooppay/invoice', '', 'SSL');
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/wooppay.tpl')) {
            ...
            return $this->load->view('default/template/payment/wooppay.tpl', $data);
        }
    }
    ...
    private function login() {
        ...
        $connect_options['trace'] = true;

        $client = new WooppaySoapClient($this->config->get('wooppay_url'), $connect_options);
    ...
    public function invoice()
    {
        ...
        try {
            if ($client = $this->login()) {
                ...
                $invoice_request->requestUrl = str_replace('&amp;', '&', $this->url->link('payment/wooppay/callback', 'order=' . $order_info['order_id'] . '&key=' . md5($order_info['order_id']), 'SSL'));
                ...
                $this->model_payment_wooppay->addTransaction(['order_id' => $order_info['order_id'], 'wooppay_transaction_id' => $invoice_data->response->operationId]);
    ...
    public function callback()
    {
        if ($this->request->get['key'] == md5($this->request->get['order'])) {
            try {
                if ($client = $this->login()) {
                    $this->load->model('payment/wooppay');
                    $operationId = $this->model_payment_wooppay->getTransactionRow($this->request->get['order']);
                    if ($operationId) {
